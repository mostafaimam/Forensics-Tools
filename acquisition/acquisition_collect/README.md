...
acquisition_collect walks a set of declarative **targets** (what to collect), pulls the
...
acquisition_collect -d E:\evidence --category EventLogs,Registry --backend vss
acquisition_collect -d /evidence --source /mnt/image_c --os windows --container zip
```
...
Known pain points in the existing tooling that acquisition_collect sets out to fix:

| Pain point | acquisition_collect's approach |
|---|---|
...
cd Forensics-Tools/acquisition/acquisition_collect
pip install -e .            # provides the `acquisition_collect` command
# or just run it in place:
python -m acquisition_collect --help
```

For field use, copy the `acquisition_collect/` package folder next to a portable Python
and run `python -m acquisition_collect`.
...
```bash
acquisition_collect --list-targets                 # targets for THIS operating system
acquisition_collect --list-targets --all-os        # every built-in target
acquisition_collect --target-info windows-prefetch # show the exact paths a target expands to
```
...
acquisition_collect -d E:\evidence

# just a few categories, into a single zip, with MD5 + SHA-256
acquisition_collect -d E:\evidence --category EventLogs,Registry,ProgramExecution \
          --container zip --hash md5,sha256

# specific targets only
acquisition_collect -d E:\evidence --targets windows-mft,windows-eventlogs --backend vss
```
...
```bash
acquisition_collect -d E:\evidence --category FileSystem,Registry --backend vss
```
...
acquisition_collect -d E:\evidence --source F:\ --os windows

# Linux image mounted at /mnt/evidence
acquisition_collect -d /cases/001 --source /mnt/evidence --os linux
```
...
| `--examiner NAME` / `--case REF` | recorded in `acquisition_collect_runinfo.json` |
...
```
<dest>/acquisition_collect_<host>_<UTC-timestamp>/
├── collection/                     (or collection.zip with --container zip)
│   └── <host>/<volume>/<original directory tree>/...
├── acquisition_collect_manifest.csv          one row per collected file (see below)
├── acquisition_collect_errors.csv            every file that could not be collected + why
├── acquisition_collect_runinfo.json          machine-readable run metadata
├── acquisition_collect_summary.txt           human-readable summary
└── acquisition_collect_console.log           full console transcript (UTC)
```

### `acquisition_collect_manifest.csv` columns
...
1. **Targets** are TOML files under `acquisition_collect/targets/` (plus any
...
```toml
# my_targets/edr.toml   →   acquisition_collect --target-dir my_targets ...
[[target]]
id          = "crowdstrike-logs"
