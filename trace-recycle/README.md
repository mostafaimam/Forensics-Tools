...
```
trace-recycle "C:\$Recycle.Bin" --csv deleted.csv
trace-recycle E:\evidence\collection --json deleted.json
trace-recycle "$IA1B2C3.docx" "D:\dump\INFO2"
```
...
cd Forensics-Tools/trace-recycle
pip install -e .              # provides the `trace-recycle` command
# or run in place:
python -m trace_recycle --help
```
...
trace-recycle "C:\$Recycle.Bin" --csv out.csv

# a triage collection, an image mount, or any folder — searched recursively
trace-recycle E:\evidence\collection --csv out.csv --json out.json

# one artefact file at a time
trace-recycle "$I7GH2K3.docx"
trace-recycle "C:\RECYCLER\S-1-5-21-...\INFO2"

# only the records that failed to parse
trace-recycle E:\evidence --errors-only
```
