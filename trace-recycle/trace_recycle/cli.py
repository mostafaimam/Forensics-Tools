...
from trace_recycle import __version__
from trace_recycle.output import (
    render_table,
    write_csv,
    write_json,
    write_jsonl,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  trace-recycle \"$IA1B2C3.txt\" \"D:\\dump\\INFO2\"\n"
        ),
    )
    ...
    p.add_argument("--version", action="version",
                   version=f"trace-recycle {__version__}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = (
        f"trace-recycle {__version__}: {result.artefacts_parsed} artefact file(s), "
