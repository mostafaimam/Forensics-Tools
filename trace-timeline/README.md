...
```
trace-timeline case/ --csv timeline.csv --html timeline.html
trace-timeline pf.csv recycle.csv --from 2024-03-01 --to 2024-03-08 --grep chrome
trace-timeline gui case/
```
...
interleaved. `trace-timeline` is that axis: an open, scriptable, diffable
...
cd Forensics-Tools/trace-timeline
pip install -e .
```
...
trace-timeline app.csv --time-field ts --message-field message --jsonl out.jsonl

# bare Unix timestamps
trace-timeline events.csv --time-field epoch --epoch --csv out.csv
```
...
## The desktop viewer (`trace-timeline gui`)

```bash
trace-timeline gui case/            # load a folder
trace-timeline gui                  # start empty, use "open…"
```
