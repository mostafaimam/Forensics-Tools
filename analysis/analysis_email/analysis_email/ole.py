...
class OleFile:
    ...
    # -- tree walk (added for MSG storage support) -------------
    def _read_entry(self, e) -> bytes:
        if e.size < self.mini_cutoff:
            return self._read_mini_chain(e.start_sector, e.size)
        return self._read_chain(e.start_sector, e.size)

    def walk(self, root_id: int | None = None, prefix: str = ""):
        """Yield (path, DirEntry) for every stream / storage under the root."""
        if root_id is None:
            root_id = self.entries[0].child if self.entries else NOSTREAM
        stack = [root_id]
        seen = set()
        while stack:
            nid = stack.pop()
            if nid in (NOSTREAM,) or nid in seen or nid >= len(self.entries):
                continue
            seen.add(nid)
            e = self.entries[nid]
            stack.append(e.left)
            ...
            yield path, e
            if e.entry_type == TYPE_STORAGE and e.child != NOSTREAM:
                yield from self.walk(e.child, path + "/")

    def tree(self) -> dict:
        """{path: DirEntry} for the whole container."""
        return {p: e for p, e in self.walk()}

    def read_path(self, path: str) -> bytes:
        for p, e in self.walk():
            if p == path and e.entry_type == TYPE_STREAM:
                return self._read_entry(e)
        raise OleError(f"stream not found: {path!r}")
