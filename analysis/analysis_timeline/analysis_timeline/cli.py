...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            "examples:\n"
            "  analysis_timeline case/ --csv timeline.csv\n"
            "  analysis_timeline pf.csv recycle.csv --from 2024-03-01 "
            "--to 2024-03-08 --grep chrome\n"
            "  analysis_timeline app.log.csv --time-field ts --message-field msg "
            "--jsonl out.jsonl\n"
        ),
    )
    ...
    p.add_argument("--version", action="version",
                   version=f"analysis_timeline {__version__}")
...
def _run_gui(argv: list[str]) -> int:
    gp = argparse.ArgumentParser(prog="analysis_timeline gui")
    ...
    from analysis_timeline.gui import run
