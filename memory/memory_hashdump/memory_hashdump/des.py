"""A from-scratch, single-block DES implementation (FIPS 46-3).

Used only for the classic (pre-Vista-scheme-compatible) SAM/LSA "RID
-keyed" hash unwrap, which every public SAM-dump tool implements the
same way: two 8-byte DES-ECB decrypts with keys derived from the
account's RID. Self-tested against the standard FIPS test vector.
"""
...
def _key_schedule(key8: bytes) -> list[list[int]]:
    bits = _bytes_to_bits(key8)
    permuted = _permute(bits, _PC1)
...
def _feistel(r: list[int], subkey: list[int]) -> list[int]:
...
def _crypt_block(block8: bytes, subkeys: list[list[int]]) -> bytes:
    bits = _permute(_bytes_to_bits(block8), _IP)
    left, right = bits[:32], bits[32:]
    for subkey in subkeys:
        new_right = [a ^ b for a, b in zip(left, _feistel(right, subkey))]
        left, right = right, new_right
    return _bits_to_bytes(_permute(right + left, _FP))


def encrypt_block(key8: bytes, block8: bytes) -> bytes:
    return _crypt_block(block8, _key_schedule(key8))


def decrypt_block(key8: bytes, block8: bytes) -> bytes:
    return _crypt_block(block8, list(reversed(_key_schedule(key8))))
