...
from memory_lsasecrets.hive.cells import (
    KeyNode,
    ValueNode,
    iter_subkey_offsets,
    iter_value_offsets,
)
from memory_lsasecrets.hive.regf import Hive as RawHive
...
class RegistryHive:
    ...
    def class_name(self, key: Key) -> str:
        """The key's registry "class" string - a rarely-used field most
        tools ignore, but which the LSA boot-key permutation is smuggled
        into (see ``bootkey.py``). Stored UTF-16LE but read back
        every-other-byte, matching how every public SAM/LSA tool decodes
        this specific field."""
        n = key.node
        if n.class_offset in (0, 0xFFFFFFFF) or not n.class_length:
            return ""
        try:
            raw = self.raw.cell_data(n.class_offset)[:n.class_length]
        except Exception:
            return ""
        return raw[::2].decode("latin-1", "replace")
