...
1. **recovery_metadata** — FAT / exFAT, ext2-4 (+ journal), HFS+, APFS; `--offset` partition auto-detection; `$ATTRIBUTE_LIST` (fragmented files); ADS extraction; live-volume input (`\\.\C:`).
2. **windows_mft** (+GUI) — `$MFT` (+ `$J`, `$Boot`, `$SDS`, `$I30`, `$LogFile`) → file-system timeline, resident data, ADS, `$FN` vs `$SI` timestomping flags. GUI: graphical `$MFT` browser (tree + record detail + hex).
