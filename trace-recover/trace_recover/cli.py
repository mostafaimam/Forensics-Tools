...
from trace_recover import __version__
from trace_recover.output import Writer
from trace_recover.scanner import ScanOptions, Source, scan
from trace_recover.signatures import all_signatures, select
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--version", action="version",
                   version=f"trace-recover {__version__}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"trace-recover {__version__}: {writer.count} object(s), "
          f"{writer.bytes:,} bytes carved from {src.size:,}-byte source",
          file=sys.stderr)
    print(f"  manifest: {args.out / 'trace-recover_manifest.csv'}", file=sys.stderr)
    return 0
