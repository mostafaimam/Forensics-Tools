...
from trace_recover.cli import main
...
def test_carve_all_from_image(tmp_path):
    ...
    rows = list(csv.DictReader((out / "trace-recover_manifest.csv").open(encoding="utf-8-sig")))
...
def test_manifest_only_writes_no_files(tmp_path):
    ...
    assert (out / "trace-recover_manifest.csv").exists()
