...
trace-prefetch E:\evidence\collection --files-csv referenced.csv
```
...
cd Forensics-Tools/trace-prefetch
pip install -e .
# or: python -m trace_prefetch --help
```
...
trace-prefetch C:\Windows\Prefetch --csv pf.csv --json pf.json

# a single file
trace-prefetch "CHROME.EXE-2E8C3F41.pf"

# every file each executable pulled in, one row per (prefetch, file)
trace-prefetch E:\evidence --files-csv referenced_files.csv

# only files that failed to parse
trace-prefetch E:\evidence --errors-only
```
