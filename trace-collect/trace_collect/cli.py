...
from trace_collect import __version__
from trace_collect.collector import Collector, CollectOptions, DirSink, ZipSink
from trace_collect.hashing import SUPPORTED as HASH_ALGS
from trace_collect.logutil import setup_logging
from trace_collect.paths import current_os, detect_host_context
from trace_collect.readers import get_reader
from trace_collect.report import Report
from trace_collect.targets import TargetError, load_builtin
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  trace-collect -d E:\\out --category FileSystem,EventLogs --backend vss\n"
            "  trace-collect -d /evidence --source /mnt/image_c --os windows\n"
            "  trace-collect -d E:\\out --targets windows-prefetch --container zip "
            "--hash md5,sha256\n"
        ),
    )
    p.add_argument("--version", action="version",
                   version=f"trace-collect {__version__}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        run_name = f"trace-collect_{host}_{stamp}"
        run_dir = args.dest / run_name
        run_dir.mkdir(parents=True, exist_ok=True)

        log = setup_logging(run_dir / "trace-collect_console.log",
        ...
        log.info("trace-collect %s  |  os=%s  backend=%s  targets=%d",
        ...
        from trace_collect.readers.base import ReadError
