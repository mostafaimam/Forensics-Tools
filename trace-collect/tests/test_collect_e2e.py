...
from trace_collect.paths import long_path
...
def test_dir_collection(tmp_path):
    ...
    run_dirs = list(dest.glob("trace-collect_*"))
    assert len(run_dirs) == 1
    run = run_dirs[0]

    rows = list(csv.DictReader((run / "trace-collect_manifest.csv").open(encoding="utf-8-sig")))
    ...
    runinfo = (run / "trace-collect_runinfo.json")
    assert runinfo.is_file()
    assert (run / "trace-collect_summary.txt").read_text().startswith("trace-collect")
...
def test_zip_collection_and_maxsize(tmp_path):
    ...
    assert rc == 0
    run = next(dest.glob("trace-collect_*"))
    ...
    errs = list(csv.DictReader((run / "trace-collect_errors.csv").open(encoding="utf-8-sig")))
